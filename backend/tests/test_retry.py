...
We monkeypatch `time.sleep` to keep the test instant — the real sleep
...
def test_retry_on_5xx_then_give_up(monkeypatch):
    monkeypatch.setattr(retrievers.time, "sleep", lambda *_: None)
    ...
    ok, result = retrievers._scroll_with_retry(boom, max_attempts=4, base_delay=0.01)
    assert ok is False
...
def test_retry_succeeds_eventually(monkeypatch):
    ...
    def flaky(*a, **k):
        ...
        # Return exactly (points_list, offset) — what qdrant_client.scroll
        # would return when the scroll is FINISHED (offset=None on the last
        # page). _scroll_with_retry unpacks these as `points, offset`.
        return (["doc1", "doc2"], None)

    ok, result = retrievers._scroll_with_retry(flaky, max_attempts=5, base_delay=0.01)
    assert ok is True
    assert attempts["n"] == 3
...
def test_fail_fast_on_401(monkeypatch):
    ...
    ok, _ = retrievers._scroll_with_retry(boom_401, max_attempts=10, base_delay=1.0)
    assert ok is False
...
def test_retry_on_429(monkeypatch):
    ...
    ok, result = retrievers._scroll_with_retry(boom_429, max_attempts=3, base_delay=0.01)
    assert ok is True
    assert calls["n"] == 2


def test_no_status_code_treated_as_transient(monkeypatch):
    ...
    ok, _ = retrievers._scroll_with_retry(boom_network, max_attempts=3, base_delay=0.01)
    assert ok is False
    assert calls["n"] == 3
